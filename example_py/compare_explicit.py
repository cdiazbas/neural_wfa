...
def run_deterministic(problem, ny, nx, regu_v=0.0, regu_qu=0.0, label="det"):
    ...
    return {
        'blos': blos, 'btrans': btrans, 'phi': phi,
        'unc_blos': unc_blos, 'unc_btrans': unc_btrans, 'unc_phi': unc_phi,
        'solver': solver
    }


def run_probabilistic(problem, sigma_obs=None, regu_blos=0.0, regu_bqu=0.0, label="prob", init_logvar=-5.0):
...
def plot_dual_dynamics(history, target_nll_extensive, title_suffix="", save_name="dual_dynamics.png"):
    ...
    if 'target' in history[0]:
        ...
        axes[0, 0].axhline(y=target_nll_extensive, color='r', linestyle='--', label='Target NLL') # Extensive
...
def plot_comparison(det_results, prob_results, title_suffix="", save_name="comparison.png"):
    ...
    blos_vmax = 500
    btrans_vmax = 500
    unc_blos_vmax = max(np.nanpercentile(det_results['unc_blos'], 95),
                        np.nanpercentile(prob_results['unc_blos'], 95))
    unc_btrans_vmax = max(np.nanpercentile(det_results['unc_btrans'], 95),
                          np.nanpercentile(prob_results['unc_btrans'], 95))
    unc_phi_vmax = max(np.nanpercentile(np.degrees(det_results['unc_phi']), 95),
                       np.nanpercentile(np.degrees(prob_results['unc_phi']), 95))
    
    # Row 0: Blos Mean
    im00 = axes[0, 0].imshow(det_results['blos'], cmap='RdBu_r', vmin=-blos_vmax, vmax=blos_vmax)
    axes[0, 0].set_title('Deterministic: Blos [G]')
    plt.colorbar(im00, ax=axes[0, 0])
    
    im01 = axes[0, 1].imshow(prob_results['blos'], cmap='RdBu_r', vmin=-blos_vmax, vmax=blos_vmax)
    axes[0, 1].set_title('Probabilistic: Blos [G]')
    plt.colorbar(im01, ax=axes[0, 1])
    
    # Row 1: Btrans Mean
    im10 = axes[1, 0].imshow(det_results['btrans'], cmap='gray', vmin=0, vmax=btrans_vmax)
    axes[1, 0].set_title('Deterministic: Btrans [G]')
    plt.colorbar(im10, ax=axes[1, 0])
    
    im11 = axes[1, 1].imshow(prob_results['btrans'], cmap='gray', vmin=0, vmax=btrans_vmax)
    axes[1, 1].set_title('Probabilistic: Btrans [G]')
    plt.colorbar(im11, ax=axes[1, 1])
    
    # Row 2: Azimuth Mean
    im20 = axes[2, 0].imshow(np.degrees(det_results['phi']), cmap='twilight', vmin=0, vmax=180)
    axes[2, 0].set_title('Deterministic: Azimuth [deg]')
    plt.colorbar(im20, ax=axes[2, 0])
    
    im21 = axes[2, 1].imshow(np.degrees(prob_results['phi']), cmap='twilight', vmin=0, vmax=180)
    axes[2, 1].set_title('Probabilistic: Azimuth [deg]')
    plt.colorbar(im21, ax=axes[2, 1])
    
    # Row 3: Blos Uncertainty
    im30 = axes[3, 0].imshow(det_results['unc_blos'], cmap='cividis', vmin=0, vmax=unc_blos_vmax)
    axes[3, 0].set_title('Deterministic: std(Blos) [G]')
    plt.colorbar(im30, ax=axes[3, 0])
    
    im31 = axes[3, 1].imshow(prob_results['unc_blos'], cmap='cividis', vmin=0, vmax=unc_blos_vmax)
    axes[3, 1].set_title('Probabilistic: std(Blos) [G]')
    plt.colorbar(im31, ax=axes[3, 1])
    
    # Row 4: Btrans Uncertainty
    im40 = axes[4, 0].imshow(det_results['unc_btrans'], cmap='cividis', vmin=0, vmax=unc_btrans_vmax)
    axes[4, 0].set_title('Deterministic: std(Btrans) [G]')
    plt.colorbar(im40, ax=axes[4, 0])
    
    im41 = axes[4, 1].imshow(prob_results['unc_btrans'], cmap='cividis', vmin=0, vmax=unc_btrans_vmax)
    axes[4, 1].set_title('Probabilistic: std(Btrans) [G]')
    plt.colorbar(im41, ax=axes[4, 1])
    
    # Row 5: Azimuth Uncertainty
    im50 = axes[5, 0].imshow(np.degrees(det_results['unc_phi']), cmap='cividis', vmin=0, vmax=unc_phi_vmax)
    axes[5, 0].set_title('Deterministic: std(phi) [deg]')
    plt.colorbar(im50, ax=axes[5, 0])
    
    im51 = axes[5, 1].imshow(np.degrees(prob_results['unc_phi']), cmap='cividis', vmin=0, vmax=unc_phi_vmax)
    ...
    plt.suptitle(f'Deterministic vs Probabilistic Comparison{title_suffix}', fontsize=14, y=0.98)
...
def print_metrics(det_results, prob_results, label=""):
    ...
    print(f"  Det : Min={np.nanmin(det_results['unc_blos']):.2f}, Max={np.nanmax(det_results['unc_blos']):.2f}, Mean={np.nanmean(det_results['unc_blos']):.2f}, Median={np.nanmedian(det_results['unc_blos']):.2f}")
    print(f"  Prob: Min={np.nanmin(prob_results['unc_blos']):.2f}, Max={np.nanmax(prob_results['unc_blos']):.2f}, Mean={np.nanmean(prob_results['unc_blos']):.2f}, Median={np.nanmedian(prob_results['unc_blos']):.2f}")
    
    print("\nSigma Statistics (Btrans):")
    print(f"  Det : Min={np.nanmin(det_results['unc_btrans']):.2f}, Max={np.nanmax(det_results['unc_btrans']):.2f}, Mean={np.nanmean(det_results['unc_btrans']):.2f}, Median={np.nanmedian(det_results['unc_btrans']):.2f}")
    print(f"  Prob: Min={np.nanmin(prob_results['unc_btrans']):.2f}, Max={np.nanmax(prob_results['unc_btrans']):.2f}, Mean={np.nanmean(prob_results['unc_btrans']):.2f}, Median={np.nanmedian(prob_results['unc_btrans']):.2f}")
    ...
    corr_blos = np.corrcoef(det_results['unc_blos'].flatten(), 
                            prob_results['unc_blos'].flatten())[0, 1]
    corr_btrans = np.corrcoef(det_results['unc_btrans'].flatten(), 
                              prob_results['unc_btrans'].flatten())[0, 1]
    
    print(f"Correlation σ(Blos):   {corr_blos:.4f}")
    print(f"Correlation σ(Btrans): {corr_btrans:.4f}")
    
    # Coverage (% of det within prob ± 2σ)
    within_2sigma_blos = np.abs(det_results['blos'] - prob_results['blos']) < 2 * prob_results['unc_blos']
    within_2sigma_btrans = np.abs(det_results['btrans'] - prob_results['btrans']) < 2 * prob_results['unc_btrans']
...
def check_noise_calibration(solver, problem, sigma_obs_used, label=""):
    ...
    with torch.no_grad():
        ...
        stokes_obs = torch.stack([
            problem.obs.stokes_Q,
            problem.obs.stokes_U,
            problem.obs.stokes_V
        ], dim=1)  # (N, 3, Nw)
    ...
    if calibration_ratio > 1.2:
        print(f"  ⚠️  WARNING: Residuals are {calibration_ratio:.1f}x larger than assumed!")
        print(f"      Model is underfitting or σ_obs is too small.")
    elif calibration_ratio < 0.8:
        print(f"  ⚠️  WARNING: Residuals are {calibration_ratio:.1f}x smaller than assumed!")
        print(f"      Model is overfitting or σ_obs is too large.")
    else:
        print(f"  ✓ Good calibration (within 20% of assumed value)")
...
if RUN_WITHOUT_REGULARIZATION:
    ...
    det_noreg = run_deterministic(problem, ny, nx, regu_v=0.0, regu_qu=0.0, label="no_reg")
    ...
    for init_val in init_vals:
        ...
        prob_noreg = run_probabilistic(problem, sigma_obs=sigma_obs, 
                                       regu_blos=0.0, regu_bqu=0.0, 
                                       label=f"no_reg_{init_val}",
                                       init_logvar=init_val)
        
        plot_comparison(det_noreg, prob_noreg, 
                        title_suffix=f" (No Reg, Init {init_val})", 
                        save_name=f"comparison_init_{init_val}_no_reg.png")
        
        n_stokes = 3 # Q, U, V only
        n_active_wav = len(problem.active_wav_idx) if problem.active_wav_idx is not None else problem.obs.n_lambda
        n_points = 35600 * n_stokes * n_active_wav
        target_extensive = 0.5 * (1.0 + np.log(0.0121**2)) * n_points
        
        plot_dual_dynamics(prob_noreg['history'], target_nll_extensive=target_extensive,
                           title_suffix=f" (No Reg, Init {init_val})",
                           save_name=f"dynamics_init_{init_val}_no_reg.png")
                           
        save_history_json(prob_noreg['history'], save_name=f"history_init_{init_val}_no_reg.json")
...
if RUN_WITH_REGULARIZATION:
    ...
    det_reg = run_deterministic(problem, ny, nx, regu_v=REGU_V, regu_qu=REGU_QU, label="reg")
    ...
    for init_val in init_vals:
        ...
        prob_reg = run_probabilistic(problem, sigma_obs=sigma_obs, 
                                 regu_blos=1.0e5, regu_bqu=0.5e5, # Retuned for extensive scale (NLL ~ 1e7)
                                 label=f"prob_reg_{init_val}",
                                 init_logvar=init_val)
        
        plot_comparison(det_reg, prob_reg,
        ...
        plot_dual_dynamics(prob_reg['history'], target_nll_extensive=target_extensive,
                           title_suffix=f" (Init {init_val})",
                           save_name=f"dynamics_init_{init_val}.png")
        
        save_history_json(prob_reg['history'], save_name=f"history_init_{init_val}.json")
