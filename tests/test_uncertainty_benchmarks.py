...
def run_comparison():
    ...
    for method in methods:
        ...
        try:
            ...
            unc_blos, unc_bt, unc_phi = model.estimate_uncertainties(
                params_subset, index=indices, method=method
            )
    ...
    print(
        f"{'Method':<12} | {'Blos (mean)':<12} | {'Bt (mean)':<12} | {'Phi (mean)':<12} | {'Ratio to Taylor (Bt)':<20}"
    )
    print("-" * 80)
    ...
    for method in methods:
        ...
        print(
            f"{method:<12} | {mean_blos:<12.4f} | {mean_bt:<12.4f} | {mean_phi:<12.6f} | {ratio_bt:<20.4f}"
        )
    ...
    for method in methods:
        if method not in results:
            continue
        print(
            f"  {method}: Blos={results[method]['blos'][0]:.4f}, Bt={results[method]['bt'][0]:.4f}"
        )
