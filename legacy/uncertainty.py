...
def estimate_uncertainties_analytical(model, params, index=None):
    ...
    chi2_Q = torch.sum(
        (model.data_stokesQ[idx_op, :] - stokesQ)[:, model.mask] ** 2.0, dim=1
    )
    chi2_U = torch.sum(
        (model.data_stokesU[idx_op, :] - stokesU)[:, model.mask] ** 2.0, dim=1
    )
    chi2_V = torch.sum(
        (model.data_stokesV[idx_op, :] - stokesV)[:, model.mask] ** 2.0, dim=1
    )
    ...
    Blos_denom = (
        n_wavelengths
        * abs(model.C * model.lin.geff) ** 2
        * torch.sum(model.dIdw[idx_op, :][:, model.mask] ** 2, dim=-1)
    )
    Blos_uncertainty = torch.sqrt(chi2_V / (Blos_denom + 1e-20))

    denom_Q = (
        n_wavelengths
        * (0.75 * model.C**2 * model.lin.Gg) ** 2
        * torch.sum(model.dIdwscl[idx_op, :][:, model.mask] ** 2, dim=-1)
    )
    ...
    uncertainty_btr = torch.sqrt(
        (dBtr_dQ**2 * BQ_uncertainty**2) + (dBtr_dU**2 * BU_uncertainty**2)
    )
    ...
    uncertainty_phib = torch.sqrt(
        (dphiB_dQ**2 * BQ_uncertainty**2) + (dphiB_dU**2 * BU_uncertainty**2)
    )
    ...
    if len(Blos_uncertainty) == n_total:
        uncertainty_blos = (
            Blos_uncertainty.reshape(model.ny, model.nx, model.nt)
            .detach()
            .cpu()
            .numpy()
        )
        ...
        uncertainty_phib = (
            uncertainty_phib.reshape(model.ny, model.nx, model.nt)
            .detach()
            .cpu()
            .numpy()
        )
        ...
        uncertainty_btr = (
            full_btr.reshape(model.ny, model.nx, model.nt).detach().cpu().numpy()
        )
        uncertainty_phib = (
...
def estimate_uncertainties_taylor(model, params, index=None):
    ...
    sensitivity_Blos = abs(model.C * model.lin.geff) * torch.sqrt(
        torch.sum(dIdw_sq, dim=1)
    )
    sigma_Blos = rms_V / (sensitivity_Blos + 1e-9)

    dIdwscl_sq = model.dIdwscl[idx_op, :][:, model.mask] ** 2
    sensitivity_BQ_BU = abs(0.75 * model.C**2 * model.lin.Gg) * torch.sqrt(
        torch.sum(dIdwscl_sq, dim=1)
    )
...
def estimate_uncertainties_pytorch(model, params, index=None):
    ...
    hessians = batch_hessian_fn(
        params, batch_Q, batch_U, batch_V, batch_dIdw, batch_dIdwscl
    )
    ...
    if len(Blos_uncertainty) == n_total:
        return (
            ...
            .cpu()
            .numpy(),
            uncertainty_btr.reshape(model.ny, model.nx, model.nt)
            .detach()
            .cpu()
            .numpy(),
            uncertainty_phib.reshape(model.ny, model.nx, model.nt)
