...
class Observation:
    ...
    def __init__(
        ...
        if active_wav_idx is not None:
            self.active_wav_idx = (
                torch.tensor(active_wav_idx, device=self.device)
                if not isinstance(active_wav_idx, torch.Tensor)
                else active_wav_idx.to(self.device)
            )
        else:
            self.active_wav_idx = torch.arange(self.n_lambda, device=self.device)
    ...
    def to(self, device):
        ...
        return Observation(
            self.data.to(device), self.wavelengths.to(device), active_list, device
        )
    ...
    def get_pixel(self, idx: Union[int, slice, torch.Tensor]) -> "Observation":
        ...
        return Observation(
            subset_data, self.wavelengths, self.active_wav_idx, self.device
        )
    ...
    def get_coordinates(self) -> torch.Tensor:
        ...
        if len(self.grid_shape) != 2:
            return torch.linspace(-1, 1, self.n_pixels, device=self.device).unsqueeze(
                -1
            )
