...
class MagneticField:
    ...
    def btrans(self) -> torch.Tensor:
        """
        Transverse magnetic field magnitude (Gauss).

        Btrans = sqrt(sqrt(B_Q^2 + B_U^2))
        """
    ...
    @staticmethod
    def polar2bqu(
        blos: torch.Tensor, btrans: torch.Tensor, phi: torch.Tensor
    ...
    @staticmethod
    def bqu2polar(
        blos: torch.Tensor, bq: torch.Tensor, bu: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
