...
class WFAProblem:
    ...
    def __init__(
        ...
        # Resolve device (fallback to observation device if None)
        self.device = resolve_device(device if device else observation.device)
    ...
    def _compute_derivatives(self):
        ...
        # Explicitly force float32 for MPS compatibility
        self.dIdw = torch.from_numpy(dIdw_np).to(self.device).float()
