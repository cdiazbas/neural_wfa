...
def _get_smoothing_kernel(device):
    ...
    weights = torch.tensor([[0.5, 1.0, 0.5], [1.0, 0.0, 1.0], [0.5, 1.0, 0.5]], device=device)
...
def mean_regularization(field_map: torch.Tensor, target_mean: float = None) -> torch.Tensor:
    """
...
def target_value_regularization(field_map: torch.Tensor, target_val: float) -> torch.Tensor:
    """
