...
def estimate_uncertainties_diagonal(
    problem: WFAProblem, field: MagneticField, indices: torch.Tensor = None
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Estimate uncertainties using the diagonal approximation of the Hessian
    ...
    # Sensitivities
    sens_blos = abs(problem.C * problem.lin.geff) * torch.sqrt(
        torch.sum(dIdw[..., mask] ** 2, dim=-1)
    )
    sigma_blos = torch.sqrt(res_V_sq / (n_lambda * sens_blos**2 + 1e-20))

    sens_bqu = abs(0.75 * problem.C**2 * problem.lin.Gg) * torch.sqrt(
        torch.sum(dIdwscl[..., mask] ** 2, dim=-1)
    )
    ...
    sigma_btrans_sq = torch.sqrt(
        ((BQ / (Btr + 1e-9)) * sigma_bq) ** 2 + ((BU / (Btr + 1e-9)) * sigma_bu) ** 2
    )
    sigma_btrans = sigma_btrans_sq / (2 * torch.sqrt(Btr + 1e-9) + 1e-9)
    sigma_phi = 0.5 * torch.sqrt(
        ((BU / (Btr_sq + 1e-9)) * sigma_bq) ** 2
        + ((BQ / (Btr_sq + 1e-9)) * sigma_bu) ** 2
    )
...
def estimate_uncertainties_pytorch(
    ...
    # (N, 3)
    p_init = (
        torch.stack([field.blos, field.b_q, field.b_u], dim=-1)
        .squeeze(1)
        .detach()
        .requires_grad_(True)
    )
    ...
    hessians = batch_hessian_fn(
        p_init, batch_Q, batch_U, batch_V, batch_dIdw, batch_dIdwscl
    )

    # Invert to get Variances
    hessians = 0.5 * (hessians + hessians.transpose(-1, -2)) + 1e-8 * torch.eye(
        3, device=p_init.device
    )
