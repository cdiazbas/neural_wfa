...
def plot_loss(output_dict):
    ...
    if (
        "lr" in output_dict
        and output_dict["lr"] is not None
        and len(output_dict["lr"]) > 0
    ):
        ax2 = plt.gca().twinx()
...
def plot_wfa_results(blos, btrans, phi, save_name=None, show=True):
    ...
    im2 = ax[2].imshow(
        phi, vmax=np.pi, vmin=0, cmap="twilight", interpolation="nearest", extent=extent
    )
    ...
    f.colorbar(
        im0, ax=ax[0], orientation="horizontal", label=names[0] + " [G]", pad=0.17
    )
    f.colorbar(
        im1, ax=ax[1], orientation="horizontal", label=names[1] + " [G]", pad=0.17
    )
    f.colorbar(
        im2, ax=ax[2], orientation="horizontal", label=names[2] + " [rad]", pad=0.17
    )
...
def plot_stokes_profiles(
    wav, obs_quv, mod_quv, mask_indices=None, save_name=None, show=True
):
    """
...
def plot_chi2_maps(
    ...
    chi2_v,
    chi2_total=None,
    save_name_components=None,
    save_name_total=None,
    show=True,
):
    """
    ...
    if chi2_total is not None:
        ...
        im = plt.imshow(
            chi2_total, cmap="magma", interpolation="nearest", extent=extent
        )
...
def plot_temporal_evolution(
    ...
    blos_1 = (
        torch2numpy(blos_series_1)
        if isinstance(blos_series_1, torch.Tensor)
        else np.asarray(blos_series_1)
    )
    btrans_1 = (
        torch2numpy(btrans_series_1)
        if isinstance(btrans_series_1, torch.Tensor)
        else np.asarray(btrans_series_1)
    )
    azi_1 = (
        torch2numpy(azi_series_1)
        if isinstance(azi_series_1, torch.Tensor)
        else np.asarray(azi_series_1)
    )
    ...
    if blos_series_2 is not None:
        blos_2 = (
            torch2numpy(blos_series_2)
            if isinstance(blos_series_2, torch.Tensor)
            else np.asarray(blos_series_2)
        )
        btrans_2 = (
            torch2numpy(btrans_series_2)
            if isinstance(btrans_series_2, torch.Tensor)
            else np.asarray(btrans_series_2)
        )
        azi_2 = (
            torch2numpy(azi_series_2)
            if isinstance(azi_series_2, torch.Tensor)
            else np.asarray(azi_series_2)
        )
    ...
    if pixel_coords is not None:
        fig.suptitle(
            f"Temporal Evolution at Pixel (y={pixel_coords[0]}, x={pixel_coords[1]})",
            fontsize=12,
            y=1.02,
        )
