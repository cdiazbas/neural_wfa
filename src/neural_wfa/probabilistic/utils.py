...
def broadcast_sigma_obs(
    ...
    if sigma_obs.ndim == 0:
        ...
        if sigma_obs.shape[0] == 1:
            ...
            raise ValueError(
                f"Expected 1 (global) or {n_stokes} (per-stokes) values, got {sigma_obs.shape[0]}"
            )
        ...
        if n_wavelengths is not None and sigma_obs.shape[1] != n_wavelengths:
            raise ValueError(
                f"Expected {n_wavelengths} wavelengths, got {sigma_obs.shape[1]}"
            )
        return sigma_obs.clone()

    else:
        raise ValueError(
            f"sigma_obs must be scalar, 1D, or 2D. Got shape {sigma_obs.shape}"
        )
