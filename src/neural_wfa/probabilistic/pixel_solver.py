...
class ProbabilisticPixelSolver:
    ...
    def initialize_parameters(self, method: str = "weak_field"):
        ...
        Initialize parameters using weak field approximation or random
        values.
        """
        with torch.no_grad():
    ...
    def solve(
        ...
        for i in t:
            ...
            for k in range(self.n_samples):
                ...
                # NLL for this sample
                nll_loss += self.compute_nll(
                    stokes_obs, stokes_pred, self.sigma_obs, mask
                )
            ...
            if regu_spatial_blos > 0:
                loss_spatial += regu_spatial_blos * smoothness_loss(
                    mu_spatial[..., 0], penalty="l2"
                )
            if regu_spatial_bqu > 0:
                loss_spatial += regu_spatial_bqu * smoothness_loss(
                    mu_spatial[..., 1], penalty="l2"
                )
                loss_spatial += regu_spatial_bqu * smoothness_loss(
                    mu_spatial[..., 2], penalty="l2"
                )
            ...
            if self.nt > 1:
                if regu_temporal_blos > 0:
                    loss_temporal += regu_temporal_blos * smoothness_loss(
                        mu_spatial[..., 0], penalty="l2_temporal"
                    )
                if regu_temporal_bqu > 0:
                    loss_temporal += regu_temporal_bqu * smoothness_loss(
                        mu_spatial[..., 1], penalty="l2_temporal"
                    )
                    loss_temporal += regu_temporal_bqu * smoothness_loss(
                        mu_spatial[..., 2], penalty="l2_temporal"
                    )
            ...
            with torch.no_grad():
                ...
                target_nll_val = (
                    0.5 * (1.0 + torch.log(current_sigma**2)) * n_data_points
                )
            ...
            if verbose and i % 50 == 0:
                print(f"\n[Iter {i}] MaxEnt State:")
                print(
                    f"  NLL:     {nll_loss.item():.4e} (Target: {target_nll_val:.4e})"
                )
        ...
        if verbose:
            ...
            if not self.sigma_obs_fixed:
                ...
                if self.sigma_obs_granularity == "per_stokes":
                    ...
                    print(
                        f"Learned σ_obs: Q={sigma_vals[0]:.4e}, U={sigma_vals[1]:.4e}, V={sigma_vals[2]:.4e}, mean={sigma_vals.mean():.4e}"
                    )
                else:
