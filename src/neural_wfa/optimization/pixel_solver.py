...
from neural_wfa.utils.misc import resolve_device
...
class PixelSolver:
    ...
    Optimizes magnetic field parameters directly for each pixel (and
    time step).
    """

    def __init__(self, problem: WFAProblem, device=None):
        self.problem = problem
        self.nt = problem.nt
        self.device = resolve_device(device, verbose=False) if device is not None else problem.device
        ...
        self.params = torch.zeros(
            self.nt, self.n_spatial, 3, device=self.device, requires_grad=True
        )
        ...
        if method == "zeros":
            ...
            with torch.no_grad():
                ...
                if Blos.numel() != self.nt * self.n_spatial:
                    print(
                        f"Warning: Initial guess shape {Blos.shape} does not match parameter shape ({self.nt}, {self.n_spatial}). Using flat fill."
                    )
        ...
        for i in t:
            ...
            if reguV > 0 or reguQU > 0:
                ...
                for t_idx in range(self.nt):
                    ...
                    if reguV > 0:
                        loss_spatial += reguV * smoothness_loss(
                            p_t[:, :, 0], penalty="l2"
                        )

                    if reguQU > 0:
                        loss_spatial += reguQU * smoothness_loss(
                            p_t[:, :, 1], penalty="l2"
                        )
                        loss_spatial += reguQU * smoothness_loss(
                            p_t[:, :, 2], penalty="l2"
            ...
            if self.nt > 1:
                ...
                if reguT_Blos > 0:
                    loss_temporal += reguT_Blos * temporal_smoothness_loss(
                        p_time[..., 0]
                    )
                if reguT_BQU > 0:
                    loss_temporal += reguT_BQU * temporal_smoothness_loss(
                        p_time[..., 1]
                    )
                    loss_temporal += reguT_BQU * temporal_smoothness_loss(
                        p_time[..., 2]
