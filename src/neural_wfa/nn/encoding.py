...
class HashEmbedder2D(nn.Module):
    ...
    Based on 'Instant Neural Graphics Primitives with a Multiresolution
    ...
    def __init__(
        ...
        self.resolutions = [
            int(np.floor(base_resolution * b**i)) for i in range(num_levels)
        ]
        ...
        for i in range(self.total_levels):
            if i < n_to_compute:
                ...
                def get_idx(coords):
                    ...
                    h = (coords[:, 0] * self.primes[1]) ^ (
                        coords[:, 1] * self.primes[2]
                    )
...
class HashEmbedder3D(nn.Module):
    ...
    Supports different resolution ranges for temporal vs spatial
    dimensions.
    """
    ...
    def __init__(
        ...
        b_spatial = np.exp(
            (np.log(max_resolution) - np.log(base_resolution)) / max(num_levels - 1, 1)
        )
        b_temporal = np.exp(
            (
                np.log(self.temporal_max_resolution)
                - np.log(self.temporal_base_resolution)
            )
            / max(num_levels - 1, 1)
        )

        self.resolutions_spatial = [
            int(np.floor(base_resolution * b_spatial**i)) for i in range(num_levels)
        ]
        self.resolutions_temporal = [
            int(np.floor(self.temporal_base_resolution * b_temporal**i))
        ...
        for i in range(self.num_levels):
            if i < n_to_compute:
                ...
                max_vals = torch.tensor(
                    [res_t - 1, res_s - 1, res_s - 1], device=x.device
                )
                x0 = torch.clamp(
                    x0,
                    min=torch.zeros(3, device=x.device, dtype=torch.long),
                    max=max_vals,
                )
                x1 = torch.clamp(
                    x1,
                ...
                def get_idx(coords):
                    ...
                    h = (
                        (t_idx.long() * self.primes[1])
                        ^ (y_idx.long() * self.primes[2])
                        ^ (x_idx.long() * self.primes[3])
                    )
                    return h % self.embeddings[i].num_embeddings
...
class HybridHashEmbedder3D(nn.Module):
    ...
    def __init__(
        ...
        spatial_bbox = (
            (bounding_box[0][1], bounding_box[0][2]),
        ...
        self.register_buffer(
            "B_temporal", torch.randn(1, temporal_m_freqs) * temporal_sigma
        )
...
class HashEmbedding(nn.Module):
    """
    Unified factory for Hash Encoding that auto-dispatches based on input
    dimensions.
    ...
    def __init__(self, dim_in: int, mode: str = "auto", **kwargs):
        ...
        if dim_in == 2:
            ...
            if mode in ["auto", "full"]:
                self.encoder = HashEmbedder3D(
                    **self._filter_kwargs(kwargs, HashEmbedder3D)
                )
            else:  # hybrid
                self.encoder = HybridHashEmbedder3D(
                    **self._filter_kwargs(kwargs, HybridHashEmbedder3D)
                )
        else:
        ...
        if hasattr(self.encoder, "output_dim"):
            ...
            self.output_dim = (
                len(self.encoder.embeddings) * self.encoder.features_per_level
            )
