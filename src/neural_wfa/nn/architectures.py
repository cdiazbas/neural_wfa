    ...
    def __init__(
        ...
        if tune_beta:
            ...
            self.register_buffer(
                "beta", torch.ones(self.num_resnet_blocks, self.num_layers_per_block)
            )
...
class TemporalMLP(nn.Module):
    """
    Temporal MLP variant using FiLM (Feature-wise Linear Modulation) for time
    information.
    """
    ...
    def __init__(
        ...
        if fourier_features:
            ...
            n_param = (
                len(sigma_spatial)
                if isinstance(sigma_spatial, list)
                else spatial_dim_in
            )
...
class HashMLP(nn.Module):
    ...
    This architecture is usually much smaller (shallower/narrower) than
    standard MLPs because the Hash Grid handles the high-frequency
    learning.
